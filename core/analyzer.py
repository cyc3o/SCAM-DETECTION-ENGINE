...
def analyze_message(message: str) -> Dict:
    ...
    1. This function is called EXACTLY ONCE per message from main.py
    2. All detector functions ONLY return data (no printing, no file I/O)
    3. NO detector may call print(), save files, or determine verdict
    4. This function aggregates all signals and returns ONE report dict
    5. Output/printing happens ONLY in main.py after this returns
    
    VIOLATION OF THESE RULES WILL CAUSE DUPLICATE OUTPUT BUG.
    
    CORE PRINCIPLE: DETECT MANY → DECIDE ONCE → OUTPUT ONCE
    ═══════════════════════════════════════════════════════════════
    """
    msg = message.lower()
    score = 0
